...
class Template
{
    ...
    public static function showButtonFilter($controllerName, $itemsStatusCount, $currentFilterStatus, $paramsSearch, $currentFilterCategory = null)
    ...
    public static function showItemStatus($controllerName, $id, $statusValue)
    {
        ...
        $xhtml = sprintf(
            '<a  data-url="%s" type="button"  name ="change-ajax-status" class="btn btn-round %s  ">%s</a>',
            $link,
    ...
    public static function showItemIsHome($controllerName, $id, $isHomeValue)
    {
        ...
        $xhtml = sprintf(
            '<a  data-url="%s" type="button"  name ="change-ajax-isHome" class="btn btn-round %s">%s</a>',
            $link,
    ...
    public static function showItemOrdering($controllerName, $id, $orderingValue)
    {
        ...
        $xhtml   = sprintf(
            '<input type="text" data-url="%s" style="width: 100px; text-align:center;"  name="change-ajax-ordering" class="form-control" value="%s">',
            $link,
            $orderingValue
        );
        return $xhtml;
    ...
    public static function showItemThumbUpload($thumbName, $thumbAlt)
    {
        ...
        foreach ($thumbName as $value) {
            $xhtml .= sprintf(
                '<img src="%s" alt="%s" style="height: 50px" class="zvn-thumb">',
                asset("uploads/$value"),
                $thumbAlt
            );
        }
    ...
    public static function showItemContact($controllerName, $id, $statusValue)
    {
        ...
        $xhtml = sprintf(
            '<a  data-url="%s" type="button"  name ="change-ajax-contact" class="btn btn-round %s  ">%s</a>',
            $link,
            $currentTemplateStatus['class'],
            $currentTemplateStatus['name']
        );
    ...
    public static function showItemContactDisabled($controllerName, $id, $statusValue)
    {
        ...
        $link          = route($controllerName . '/contact', ['contact' => $statusValue, 'id' => $id]);
        $xhtml = sprintf(
            '<p   readonly="readonly" class="btn btn-round %s  ">%s</p>',
            $currentTemplateStatus['class'],
            $currentTemplateStatus['name']
        );
        return $xhtml;
    ...
    {
        $xhtml = sprintf(
            '<img src="%s" alt="product" style="width: 400px; height: 200px">',
            asset("uploads/$thumbName")
        );
        return $xhtml;
    }
    ...
    {
        $xhtml = sprintf(
            '<img src="%s" alt="%s" style="height:5em">',
            asset("images/$controllerName/$thumbName"),
            $thumbAlt
        );
        return $xhtml;
    ...
    public static function showPrice($item)
    {
        ...
        if (!empty($item['price_custom'])) {
            $customPrices = json_decode($item['price_custom'], true);
            $customPrices = json_decode($customPrices['value'], true);
            $priceValue = $customPrices[0];
        }

        if (empty($item['value_promotion']) && $item['promotion'] == 'default') { // không khuyến mãi
            $priceOld = number_format($priceValue) . ' VNĐ';
            $price    = number_format($priceValue) . ' VNĐ';
        } else { // có khuyến mãi
            if ($item['promotion'] == 'direct') {
                $priceOld = number_format($priceValue) . ' VNĐ';;
                $price    = number_format($priceValue - $item['value_promotion']) . ' VNĐ';
            }
            if ($item['promotion'] == 'percent') {
                $priceOld = number_format($priceValue) . ' VNĐ';
                $price    = number_format($priceValue - ($priceValue / $item['value_promotion'])) . ' VNĐ';
            }
        }
